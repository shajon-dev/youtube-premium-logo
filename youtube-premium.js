...
// @namespace    https://github.com/shajon-dev/youtube-premium-logo
// @version      1.0
// @description  Replaces the logo with a positioned Premium logo AND removes "Get Premium" upsells/banners.
// @author       SHAH MAKHDUM SHAJON
// @match        *://*.youtube.com/*
// @grant        GM_addStyle
// @run-at       document-start
// @homepageURL  https://github.com/shajon-dev/youtube-premium-logo
// @downloadURL  https://raw.githubusercontent.com/shajon-dev/youtube-premium-logo/main/youtube-premium.js
// @updateURL    https://raw.githubusercontent.com/shajon-dev/youtube-premium-logo/main/youtube-premium.js
// ==/UserScript==
...
(function() {
    ...
    // ====================================================================
    // PART 1: LOGO DATA (Base64 SVGs)
    // ====================================================================
    const svgSource = `
    <svg xmlns:dc="http://purl.org/dc/elements/1.1/" xmlns:cc="http://creativecommons.org/ns#" xmlns:rdf="http://www.w3.org/1999/02/22-rdf-syntax-ns#" xmlns:svg="http://www.w3.org/2000/svg" xmlns="http://www.w3.org/2000/svg" id="SVGRoot" version="1.1" viewBox="0 0 846 174" height="80px" width="391px">
      <defs id="defs855"><style id="style2" /></defs>
      <g id="layer1">
        <g transform="translate(0,0.36)" data-name="Layer 2" id="Layer_2">
          <g data-name="Layer 1" id="Layer_1-2" PLACEHOLDER_FOR_DARKMODE>
    ...
    // Base64 Conversions
    const logoLight = "data:image/svg+xml;base64," + btoa(svgSource);
    const svgDarkSource = svgSource.replace('PLACEHOLDER_FOR_DARKMODE', 'fill="#FFFFFF"');
    const logoDark = "data:image/svg+xml;base64," + btoa(svgDarkSource);

    // ====================================================================
    // PART 2: COMBINED CSS (Logo + Premium Remover)
    // ====================================================================
    const combinedCSS = `
        /* --- 1. LOGO REPLACEMENT STYLES --- */
        /* HIDE ORIGINAL SVG ELEMENTS */
        ...
        ytd-topbar-logo-renderer #country-code {
            ...
            /* UPDATED: -12px top for precise height */
            top: -12px !important;
        ...
        /* THEME MAPPING */
        ytd-topbar-logo-renderer #logo, ytd-masthead #masthead-logo a {
            background-image: url('${logoDark}') !important;
        }
        html:not([dark]) ytd-topbar-logo-renderer #logo,
        html:not([dark]) ytd-masthead #masthead-logo a {
            background-image: url('${logoLight}') !important;
        }


        /* --- 2. PREMIUM REMOVER STYLES (Method 1) --- */
        /* HIDE: Sidebar "YouTube Premium" Link */
    ...
    // ====================================================================
    // PART 3: JS LOGIC (Fallbacks & XPath)
    // ====================================================================

    // FUNCTION A: JS Fallback for Generic Remover (Old Browsers/ShadowDOM)
    function jsFallbackRemover() {
        const premiumLinks = document.querySelectorAll('a[href="/premium"], a[href="/paid_memberships"]');
    ...
    // FUNCTION B: XPath Remover (The Specific Backup)
    function xpathRemover() {
        const targetXPath = "/html/body/ytd-app/div[1]/tp-yt-app-drawer/div[2]/div/div[2]/div[2]/ytd-guide-renderer/div[1]/ytd-guide-section-renderer[4]/div/ytd-guide-entry-renderer[1]";
        ...
        if (element) {
            ...
            if (element.style.display !== "none" && getComputedStyle(element).display !== "none") {
                ...
                console.log("Tampermonkey: XPath Remover caught the line.");
            }
        }
    }
